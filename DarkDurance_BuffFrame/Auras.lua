...
local function makeup(self)
    self.icon = _G[self:GetName() .. 'Icon']
    self.border = _G[self:GetName() .. 'Border']

    self.overlay = self:CreateTexture(nil, 'OVERLAY')
    self.overlay:SetTexture[[Interface\AddOns\DarkDurance_BuffFrame\SmartName]]
    self.overlay:SetPoint('TOPLEFT', self, -1, 1)
    self.overlay:SetPoint('BOTTOMRIGHT', self, 1, -1)
...
local function updateDuration(self)
    local str, nextUpdate = formatTime(self.timeLeft)
...
local function auraButtonUpdate(name, index, filter)
    local btn = _G[name .. index]
    if(not btn) then return end
    if(not btn.dd_hooked) then
        btn.dd_hooked = true
        makeup(btn)
    end
...
hooksecurefunc('AuraButton_Update', auraButtonUpdate)
